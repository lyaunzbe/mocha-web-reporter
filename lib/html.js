...
function HTML(runner) {
  ...
  runner.on('end', function(){
    console.log('<ul id="mocha-stats"><li class="progress"><canvas width="40" height="40"></canvas></li><li class="passes">passes: <em>%s</em></li><li class="failures">failures: <em>%s</em></li><li class="duration">duration: <em>%s</em>s</li></ul>', stats.passes, stats.failures, (stats.end - stats.start) / 1000);
    console.log('<ul id="mocha-report">' + result + '<ul>');
  });
  ...
  runner.on('pass', function(test){
    result += util.format('%s  <li class="test pass %s">\n', indent(), test.speed);
    ++indents;
